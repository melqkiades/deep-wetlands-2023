...
def validation_loss(loader, model, loss_fn):
    ...
    with torch.no_grad():
      ...
      for (data, y) in loader:
        ...
        ##targets =  new_mask(y.unsqueeze(1).numpy())
...
def new_mask(arr_y):
    ...
    arr_z[arr_z == 1] = 0
    arr_z[arr_z == -1] = 1
    arr_fin = np.concatenate((arr_y,arr_z), axis=1)
    return arr_fin
...
def check_accuracy(loader, model, device="cuda"):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            #print("DIM orig targets", y.shape)
            #print("DIM orig targets unsqueezed", y.unsqueeze(1).shape)
            ##print("DIM new targets", targets.shape)
            ##print("DIM preds", preds.shape)
            #print("num_correct", (preds == targets).shape)
            ##print()

            ##num_correct += (preds == targets).sum()
            ##num_correct += torch.sum(preds == targets,dim = (0,1,2,3))

            #y = y.to(device).unsqueeze(1)
            #target = torch.argmax(target, dim=1)
            
            ##y = [elem.to(device).unsqueeze(1) for elem in y]

            #for elem in y:
            #    elem = elem.to(device).unsqueeze(1)
            ...
            dice_score += (2 *torch.sum((preds * y),dim = (0,1,2,3)) / (
               torch.sum((preds + y),dim = (0,1,2,3))) + 1e-8
            )
            
            IoU = BinaryJaccardIndex(ignore_index= 255).to(DEVICE)
            ##IoU = MulticlassJaccardIndex(num_classes=2).to(device)
            #metric=MulticlassJaccardIndex(...).to(device)
            score = IoU(preds,y)
            ##score = computeIOU(preds,y)

            #hist = c._fast_hist(preds,y)
            #score = c.jaccard_index(hist)[0]
...
def save_predictions_as_imgs(
    ...
    for idx, (x, y) in enumerate(loader):
        ...
        if idx % (len(loader)-1) == 2 and idx != 0:
...
##def train_fn(loader, model, optimizer, loss_fn, epoch, scheduler):
    ...
    for batch_idx, (data, y) in enumerate(loop):
        ...
        ##targets = y.unsqueeze(1).to(device=DEVICE)

        # forward
        with torch.cuda.amp.autocast():
            predictions = model(data)
            
            ##targets = torch.squeeze(tragets, 1)
            print()
            print("predictions", predictions.shape)
            print("mask", targets.shape)
           # print("mask long", targets.long())
            print()
    ...
    print("Learning rate shape", len(lr))
...
BATCH_SIZE =16#15# mixed #22 1_R #18 2_R # C 16
NUM_EPOCHS = 3
NUM_WORKERS = 2
...
PIN_MEMORY = True
LOAD_MODEL = False
#TRAIN_IMG_DIR = "#"ETCI/train_images/" # "data/train_images/"
#TRAIN_MASK_DIR = #"ETCI/train_masks/" # "data/train_masks/"
#VAL_IMG_DIR = #"ETCI/val_images/" # "data/val_images/"
...
#    )
 ...
 # train_data = sen.load_flood_train_data(IMG_DIR, MASK_DIR)
  train_data = sen.load_flood_train_data('S1/', 'Labels/')
  print("TEST",len(train_data))
  train_dataset = sen.InMemoryDataset(train_data, sen.processAndAugment)
  print("TEST2",len(train_dataset))
  train_loader = DataLoader(
        train_dataset,
        batch_size=BATCH_SIZE,
        shuffle=True,
  ...
  #valid_data = sen.load_flood_valid_data(IMG_DIR, MASK_DIR)
  valid_data = sen.load_flood_valid_data('S1/', 'Labels/')
  valid_dataset = sen.InMemoryDataset(valid_data, sen.processTestIm)
  ...
  for epoch in range(NUM_EPOCHS):
    ...
    ##train_loss, lr = train_fn(train_loader, model, optimizer, loss_fn, epoch, scheduler)
    train_acc, train_dice, train_IoU = check_accuracy(train_loader, model, device=DEVICE)
    ##train_acc, train_IoU = check_accuracy(train_loader, model, device=DEVICE)
    ...
    ##val_acc, val_IoU = check_accuracy(val_loader, model, device=DEVICE)

      # print some examples to a folder
    #pred_img, in_img, orig = save_predictions_as_imgs(val_loader, model, folder="saved_images/", device=DEVICE)
    pred_img, in_img = save_predictions_as_imgs(val_loader, model, folder="saved_images/", device=DEVICE)
