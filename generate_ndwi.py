##Code adapted from original source: https://github.com/melqkiades/deep-wetlands/blob/master/wetlands/generate_ndwi.py
...
def full_cycle():
    ...
    tif_files = os.getenv('MASK_TIFF_FILE_test')#os.getenv('NDWI_TIFF_FILE_train')
    ...
    for filename in os.listdir(tif_files):
        path = str(tif_files) + '/' + filename
        tiles = geo_utils.get_tiles(region_name, path, filename, geoboundary, patch_size)
        export_ndwi_mask_data(tiles, path)
