#Code adapted from original source: https://github.com/melqkiades/deep-wetlands/blob/master/wetlands/geo_utils.py
...
def generate_tiles(tif_dir,tif_file, output_file, area_str, size):
    ...
    with tqdm(total=width * height) as pbar:
        for w in range(0, width, size):
            for h in range(0, height, size):
                ...
                uid = '{}-{}'.format(tif_file.replace(".tif",''), index)
...
def get_tiles(shape_name, tif_dir, tif_file, geoboundary, size):

    cwd = os.getenv("CWD_DIR")
    output_file = cwd + '/{}.geojson'.format(shape_name)
    tiles = generate_tiles(tif_dir,tif_file, output_file, shape_name, size)
